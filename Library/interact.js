...
const returnBook = async (contract, bookId) => {
	await contract.returnBook(bookId);
};
...
const run = async function (contract, wallet) {
	await createBook(contract);
	const booksCount = await getBooksCount(contract);

	const allBooks = await getAllBooks(contract, booksCount);
	const availableBooks = getAvailableBooks(allBooks);

	await borrowBook(contract, availableBooks);
	const bookId = allBooks[0].id;
	let isAvailable = await isBookAvailable(contract, bookId);
	console.log(`Is this book available: ${isAvailable}`);

	await returnBook(contract, bookId);
	isAvailable = await isBookAvailable(contract, bookId);
	console.log(`Is this book available: ${isAvailable}`);
};
